...
{
    public class DodgeHandler : ICommandHandler
    {
        public Packet Packet { get; private set; }
        public Client Client { get; private set; }
        public Json.DodgeHandler JsonData { get; private set; }
        ...
        public void Init(Packet packet)
        {
            ...
            {
                throw new Exception($"[{packet.Server.Time}] Client with remoteEp '{packet.RemoteEp}' sent a dodge but has never joined.");
            }
            ...
            Logs.Log($"[{packet.Server.Time}] Received dodge packet (({Id}){X},{Y},{Z},{Rx},{Ry},{Rz},{Rw}) from '{packet.RemoteEp}'.");

            Packet packetToSend = new Packet(Packet.Server, 512, packet.Data, packet.RemoteEp);
            packetToSend.Broadcast = true;
            packetToSend.AddHeaderToData(false, Commands.ClientDodged);
            ...
            JsonData = new Json.DodgeHandler()
            {
                X = this.X,
                Y = this.Y,
                Z = this.Z,
